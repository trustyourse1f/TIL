...
시리즈 객체는 넘파이 배열(ndarray)의 하위 클래스  
...
- 'read_확장자'함수로 데이터 바로 로딩
  - csv아 xlsx등 스프레드시트형 확장자 파일에서 데이터 로딩

- 데이터프레임을 직접 생성
  - 딕혀너리 타입 데이터에서 키는 열 이름, 값은 시퀀스형 데이터 타입을 넣어 각 열의 데이터로 만듦
  - 데이터 생성 시, 열 이름을 한정하면 해당 열만 추출 가능
  - 데이터가 존재하지 않는 열을 추가하면 해당 열에은 NaN 값들 추가
